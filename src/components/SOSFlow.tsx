...
export const SOSFlow = ({ onClose, isOffline, initialType }: SOSFlowProps) => {
  ...
  const handleSubmit = async () => {
    ...
    const guestUid = localStorage.getItem('e_secours_guest_uid') || `guest_${draftId}`;
    if (!localStorage.getItem('e_secours_guest_uid')) {
      localStorage.setItem('e_secours_guest_uid', guestUid);
    }
    
    const localProfileRaw = localStorage.getItem('e_secours_guest_profile');
    let localProfile: any = null;
    if (localProfileRaw) {
      try { localProfile = JSON.parse(localProfileRaw); } catch(e) {}
    }
    const profileToUse = userProfile || localProfile;

    const incidentData: any = {
      id: draftId,
      userId: auth.currentUser?.uid || guestUid,
      userName: auth.currentUser?.displayName || profileToUse?.fullName || 'Citoyen Invité',
      userEmail: auth.currentUser?.email || profileToUse?.email || 'citoyen.invite@e-secours.bj',
      ...
      medicalInfo: includeMedicalInfo ? (profileToUse?.medicalInfo || null) : null,
      age: includeMedicalInfo ? (profileToUse?.age || null) : null,
      sex: includeMedicalInfo ? (profileToUse?.sex || null) : null,
      weight: includeMedicalInfo ? (profileToUse?.weight || null) : null,
      bloodType: includeMedicalInfo ? (profileToUse?.bloodType || null) : null,
      allergies: includeMedicalInfo ? (profileToUse?.allergies || null) : null,
      medications: includeMedicalInfo ? (profileToUse?.medications || null) : null,
      emergencyContact: includeMedicalInfo ? (profileToUse?.emergencyContact || null) : null,
      userFullName: includeMedicalInfo ? (profileToUse?.fullName || profileToUse?.displayName || 'Citoyen Invité') : null,
      userPhone: includeMedicalInfo ? (profileToUse?.phoneNumber || null) : null,
      userAddress: includeMedicalInfo ? (profileToUse?.address || null) : null,
    };

    // Store in local history for instant display in history tab
    try {
      const existingHistoryRaw = localStorage.getItem('e_secours_local_history');
      ...
      localStorage.setItem('e_secours_local_history', JSON.stringify(existingHistory.slice(0, 30)));
    } catch (e) {
