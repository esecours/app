...
export const Home = ({ onSOS, onNavigate, isAdmin, isOperator, isLoggedIn }: HomeProps) => {
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          ...
          {isAdmin && (
            <QuickServiceCard 
              icon={Shield} 
              title="Admin" 
              description="Tableau de bord & Alertes"
          ...
          {isOperator && !isAdmin && (
            <QuickServiceCard 
              icon={Shield} 
              title="Gestion" 
              description="Suivi des alertes usagers"
              colorClass="bg-purple-50 text-purple-600 border-purple-100"
              onClick={() => onNavigate('admin')}
            />
          )}
      ...
      {/* About / Contact Mini Slots */}
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        <button 
          onClick={() => onNavigate('about')}
          className="flex items-center justify-center gap-2 p-3 sm:p-4 bg-white border border-gray-100 rounded-3xl text-gray-500 hover:text-blue-600 hover:border-blue-100 transition-all font-black text-[10px] sm:text-xs tracking-widest uppercase"
        >
          <Info size={16} />
          À PROPOS
        </button>
